...
// This code is in the iQue source's 'game.c', behind a "DEVELOP" flag
void stub_level_update_1(void) { // CheckReset
#ifdef DEBUG    
    if (!gWarpTransition.isActive && (gPlayer1Controller->buttonDown == (Z_TRIG | L_CBUTTONS | R_CBUTTONS | START_BUTTON))
    && (gPlayer1Controller->buttonPressed & START_BUTTON)) {
        reset_dialog_render_state();
        if (gDebugLevelSelect	)  fade_into_special_warp(-9, 1); // -9 is PL_RETURN_SELECT
                         else  fade_into_special_warp(-2, 1); // -2 is PL_RETURN_TITLE
    }
#endif
}
...
// More code from 'game.c', this time behind a "DEBUGSW" flag
#ifdef DEBUG
...
s32 init_level(void) {
    ...
    // No disabling of sound banks during the intro so the ambient sounds play during it
